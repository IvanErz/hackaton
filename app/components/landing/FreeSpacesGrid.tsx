...
function StatusBadge({ status }: { status: ParkingLocation["status"] }) {
  if (status === "open") {
    return (
...
export function FreeSpacesGrid({ locations }: Props) {
  return (
    <section id="spots" className="scroll-mt-20 px-4 py-16 sm:px-6 sm:py-20">
      <div className="mx-auto max-w-6xl">
        ...
        <p className="mt-2 max-w-2xl text-zinc-600 dark:text-zinc-400">
          Mock data for UI—wire these cards to your API when ready.
        </p>
          ...
          {locations.map((loc) => (
            ...
            >
              ...
              <div className="mt-4 flex items-baseline gap-1">
                ...
                <span className="text-sm font-medium text-zinc-600 dark:text-zinc-400">free</span>
              </div>
